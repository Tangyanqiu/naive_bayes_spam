...
#     def __init__(self, channel, reduction=16):    ###reduction为啥是16
...
#     def forward(self, x):
#         y = self.avg_pool(x)  #先全局平均池化
#         y = self.conv_du(y)  #此时y维度为：1*1*c，刻画c个feature的权重，类似一个标量
#         return x * y
...
class Flatten(nn.Module):
    def forward(self, x):
        return x.view(x.size(0), -1)   #为了将前面多维度的tensor展平成一维
...
class ChannelGate(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=16, pool_types=['avg', 'max']):
        ...
        self.mlp = nn.Sequential(
            Flatten(),
            nn.Linear(gate_channels, gate_channels // reduction_ratio), #输入神经元个数：gate_channels，隐藏层神经元个数:gate_channels // reduction_ratio
            nn.ReLU(),
            nn.Linear(gate_channels // reduction_ratio, gate_channels) #输出神经元个数：gate_channels
            )
...
class SpatialGate(nn.Module):
    def __init__(self):
        ...
        self.spatial = BasicConv(2, 1, kernel_size, stride=1, padding=(kernel_size-1) // 2, relu=False) #7*7卷积
    def forward(self, x):
