...
def _ms_loop(dataset, index_queue, data_queue, collate_fn, scale, seed, init_fn, worker_id):
    ...
    while True:
        ...
        try:
            ...
            data_queue.put((idx, _utils.ExceptionWrapper(
                sys.exc_info())))  # data_queue.put((idx, ExceptionWrapper(sys.exc_info())))
        else:
...
class _MSDataLoaderIter(DataLoaderIter):
    def __init__(self, loader):
        ...
        if self.num_workers > 0:
            ...
            self.worker_result_queue = multiprocessing.Queue()  # multiprocessing.SimpleQueue()
...
class MSDataLoader(_DataLoader):
    def __init__(
